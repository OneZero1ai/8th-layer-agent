      ...
      {open && (
        ...
        >
          <motion.div
            ...
            style={{
              boxShadow: "0 60px 120px rgba(0,0,0,0.7), 0 0 0 1px rgba(255,179,71,0.30), 0 0 80px rgba(255,179,71,0.18)",
            }}
          >
            {/* Top ribbon */}
            <div
              ...
              style={{
                background: "linear-gradient(90deg, transparent, #FFB347 50%, transparent)",
              }}
            />
            ...
            <div className="px-8 pt-8 pb-6">
              ...
              <p
                ...
                style={{ fontFamily: "'JetBrains Mono', ui-monospace, monospace" }}
              >
                Signing opens a redacted, summary-only path between these two L2s.
                Knowledge bodies remain inside the responding enterprise.
...
function PartyTile({ enterprise, group }: { enterprise: string; group: string }) {
  const tint =
    enterprise === "orion"
...
function SignatureLine({ signed, signing }: { signed: boolean; signing: boolean }) {
  ...
  return (
    <div className="mt-6 rounded border border-white/10 bg-black/40 p-4">
      ...
      <svg viewBox="0 0 480 60" className="h-12 w-full">
        <line x1="0" y1="50" x2="480" y2="50" stroke="rgba(255,255,255,0.18)" strokeWidth="1" />
        <motion.path
          ref={pathRef}
