...
import type { DsnCandidate, DsnResolveResponse } from "../fixtures/dsn.fixture"
    ...
    <div data-testid="dsn-search" className="relative w-full">
      <div className="flex items-center gap-3">
        <div
          ...
          style={{
            boxShadow:
              phase !== "idle" ? "0 0 28px rgba(124,92,255,0.32)" : "none",
          }}
        >
      ...
      <AnimatePresence>
        {phase !== "idle" && (
          <motion.div
            ...
            style={{
              boxShadow:
                "0 30px 60px rgba(0,0,0,0.6), 0 0 0 1px rgba(124,92,255,0.18)",
            }}
          >
...
function PhaseTimeline({
  ...
  phase: DsnPhase
  response: DsnResolveResponse | null
}) {
    ...
    <div className="px-4 py-3">
      ...
      <div className="mt-3 flex items-end gap-1">
        ...
        <div className="flex h-9 flex-1 items-end gap-[2px]">
          {(embedding.length > 0 ? embedding : Array(16).fill(0)).map(
            (v, i) => {
      ...
      {top.map((c, i) => (
        <motion.div
          ...
          style={{
            ...
            borderColor:
              i === 0 ? "rgba(124,92,255,0.55)" : "rgba(255,255,255,0.10)",
          }}
        >
...
function PolicyBadge({
  ...
  policy: DsnCandidate["policy_if_queried"]
}) {
  const map = {
