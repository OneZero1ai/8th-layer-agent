...
import { LeftRail, type LayerKey } from "../network/components/LeftRail";
...
import { DsnSearchPanel, type DsnPhase } from "../network/components/DsnSearchPanel";
...
import { layoutTopology, findNode } from "../network/layout";
import {
  demoTraceFixtures,
  type DemoScenario,
...
function findL2(topology: TopologyResponse | null, l2_id: string | null): TopologyL2 | null {
...
export function NetworkPage({ initialData }: NetworkPageProps = {}) {
  ...
  const [packetTrails, setPacketTrails] = useState<
    Array<{ from: string; to: string; tone: "info" | "blocked" | "success"; label?: string }>
  >([]);
  const [zoomTo, setZoomTo] = useState<{ cx: number; cy: number; scale: number } | null>(null);
  const [flashCenter, setFlashCenter] = useState<{ x: number; y: number } | null>(null);
  ...
  const runScenario = useCallback(
    async (scenario: DemoScenario) => {
      ...
      if (firstL2 && data) {
        ...
        if (node) {
          if (scenario === "cross-enterprise-consented" || scenario === "cross-enterprise-blocked") {
            setZoomTo({ cx: 800, cy: 450, scale: 1 });
          } else {
            setZoomTo({ cx: node.enterprise === "orion" ? 460 : 1140, cy: 450, scale: 1.2 });
          }
        }
      }
      ...
      const hops: Array<{ from: string; to: string; latency: number; label: string }> = [];
  ...
  useEffect(() => () => clearTraceTimers(), []);

  // DSN phase handler: highlights L2s while DSN resolves.
  const onDsnPhase = useCallback((phase: DsnPhase, payload: { topL2Ids: string[]; intent: string } | null) => {
    if (phase === "idle") {
      setDsnHighlight([]);
      return;
    }
  ...
  const finalHighlightNodes = highlightedNodeIds.length > 0 ? highlightedNodeIds : dsnNodeHighlights;

  const tourZoom = tourState?.zoomTo ?? null;
  const finalZoom = tourZoom ?? zoomTo;

  const selectedL2 = findL2(data, selectedL2Id);
  const hasCrossEntConsent =
    (data?.cross_enterprise_consents.length ?? 0) > 0;
  ...
  return (
    <DesktopOnlyGate>
      ...
      >
        <TopBar topology={data} lastUpdated={poll.lastUpdated} pollError={poll.error} />
          ...
          <div className="relative flex-1">
            {!data ? (
              ...
              <>
                ...
                <L2DetailPanel l2={selectedL2} onClose={() => setSelectedL2Id(null)} />
                <PacketTraceOverlay
